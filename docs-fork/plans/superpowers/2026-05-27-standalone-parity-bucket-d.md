...
| `src/standalone/__tests__/request-service-bridge.test.ts` | 3 | **Create** | response→ok, error→handler-error, event→emitEvent (no resolve), unknown→unknown-method |
| `src/standalone/dispatcher.ts` | 3 | Modify | `DispatchContext.emitEvent?`; new service-bridge tier before the allowlist gate |
...
const LLM_ENV_KEYS = ['ANTHROPIC_API_KEY', 'OPENAI_API_KEY', 'COACH_LLM_MODEL', 'COACH_LLM_BASE_URL', 'COACH_LLM_MAX_TOKENS'];
...
describe('callLlmJson OpenAI strict-mode self-heal (no code change — fallback covers it)', () => {
  it('drops modelOptions and retries in plain mode after a response_format 400', async () => {
    ...
    // Attempt 0 carried response_format; attempt 1 dropped it.
...
> Note: the `callLlmJson` test imports the real `panel-llm` (the `vscode` it pulls resolves to the stub via the vitest alias in `vitest.config.mts:36`). It exercises the stub `lm` you build in Task 2, so this single test is **expected to stay red until Task 2 is done** — that is fine; it lives here because it is a provider-shaping concern. The `detectProvider`/`mergeSameRole`/provider-shaping tests above pass at the end of Task 1.
...
class AnthropicProvider implements LlmProvider {
  ...
  constructor(key: string, env: NodeJS.ProcessEnv) {
    ...
    this.maxTokens = Number(env.COACH_LLM_MAX_TOKENS) || 8192;
  }
...
- [ ] **Step 4: Run the test to verify the provider-level tests pass**

Run: `npx vitest run src/standalone/__tests__/llm-provider.test.ts -t "mergeSameRole|detectProvider|Anthropic provider|OpenAI provider"`
Expected: PASS. (The `callLlmJson` strict-mode test stays red until Task 2 — it depends on the stub `lm`.)
...
- [ ] **Step 4: Run the stub + the deferred provider test to verify they pass**

Run: `npx vitest run src/standalone/__tests__/vscode-stub.test.ts src/standalone/__tests__/llm-provider.test.ts`
Expected: PASS — including the `callLlmJson` OpenAI strict-mode self-heal test from Task 1 (now reachable through the stub `lm`).
...
**Ordering coupling with bucket A:** this plan assumes bucket A's two additions (`getDataExplorer`, `evaluateExpression`) have landed, so the baseline is **42 → 45**. If bucket A has *not* landed in this tree, the baseline is 40 → 43 and the bucket-A membership assertions below will not yet be present — adjust the count and drop those two assertions. Confirm the current size before editing: `node -e "console.log(require('./src/standalone/v1-allowed').V1_ALLOWED?.size)"` is unreliable under TS; instead read the count asserted in the current `v1-allowed.test.ts` (`toBe(42)` ⇒ bucket A landed ⇒ target 45).
...
## Task 6: Smoke — Rule Playground NL→rule compiles
...
Expected: PASS — including the new NL→rule test. (If bucket A's in-flight smoke work makes the suite red for unrelated reasons, run just this test: `playwright test --config=tests/standalone/playwright/playwright.config.ts -g "compiles a natural-language rule"`.)

- [ ] **Step 3: Commit**

```bash
git add tests/standalone/playwright/smoke.spec.ts
git commit -m "test(standalone): smoke coverage for Rule Playground NL->rule compile"
```
...
Run: `npx vitest run src/standalone/__tests__/request-service-bridge.test.ts`
Expected: FAIL — cannot resolve `../request-service-bridge`.
...
export function dispatchServiceMethod(
  ...
  return new Promise<DispatchResult>((resolve) => {
    try {
      ...
      const captureWebview = {
        postMessage: (frame: ResponseFrame): void => {
          ...
          if (data && typeof data === 'object' && data.error) {
            resolve({ ok: false, error: { code: 'handler-error', method, message: String(data.error) } });
          } else {
      ...
      resolve({ ok: false, error: { code: 'handler-error', method, message: err instanceof Error ? err.message : String(err) } });
    }
  });
}
```
...
```bash
git add src/standalone/request-service-bridge.ts src/standalone/__tests__/request-service-bridge.test.ts
git commit -m "feat(standalone): add request-service-bridge (fresh capturing PanelRequestService per call)"
```
...
The service-bridge tier sits **after** native, **before** the allowlist gate, with **no data-ready guard** (the data-needing service handlers self-guard — `reviewContextFiles` → "Analyzer not ready"; `triageSkills`/`triageCatalog` degrade via `getUserContext()` to empties — and a tier guard would replace `reviewContextFiles`'s specific message with the generic "data not ready"). `DispatchContext` gains an optional `emitEvent` so existing call sites and tests that pass only `{ analyzer, parseResult }` keep compiling.
...
(The existing data-ready guard and registry lookup below stay unchanged — they remain "Tier 3a/3b" for registry methods including the NL-rule methods.)
...
git commit -m "feat(standalone): add service-bridge dispatcher tier + DispatchContext.emitEvent"
```
...
Run: `npm run build` then `npx vitest run --config tests/standalone/integration/vitest.config.ts -t "service bridge|reviewProgress event"`
...
allowlisted here). With no API key, panel-llm methods surface a descriptive "No language
model available" error; `compileNlRule` silently falls back to a heuristic template.
```

- [ ] **Step 2: Pack check — the provider + bridge ship in the existing bundle**

Run: `npm run build` then `npm run pack:check`
Expected: the `npm pack --dry-run` listing includes `dist/standalone/cli.js` (covered by `package.json#files` `dist/standalone/`). No new bundle entry is required — `llm-provider.ts`, `request-service-bridge.ts`, and `v1-service-allowed.ts` compile into the existing standalone CLI bundle via the stub alias and the dispatcher imports.
...
Run: `git diff --name-only abc0a6c -- src/ | Select-String -NotMatch '^src/standalone/'`
Expected: prints nothing. Every `src/` change is under `src/standalone/`: new `llm-provider.ts`, `request-service-bridge.ts`, `v1-service-allowed.ts`; modified `vscode-stub.ts`, `dispatcher.ts`, `v1-allowed.ts`, `server.ts`, `webview-shim.ts`; plus `src/standalone/__tests__/*`. `panel-llm.ts`, `core/rule-compiler.ts`, `panel-rpc.ts`, `panel-request-service.ts` are **unchanged**. Use baseline `abc0a6c`, not `upstream/main`. If any upstream `src/` file outside `src/standalone/` appears, stop and revert it.
...
- **OpenAI strict-mode self-heal** is unit-covered (Task 1) via `callLlmJson` + `SCHEMA_CONTEXT_REVIEW` + a mocked 400-then-200 `fetch` — no workspace seeding needed, and it proves the "no code change; the existing fallback covers it" claim directly.
...
- **Bucket-A coupling:** Task 4 edits the same `_inner` set and the same `v1-allowed.test.ts` size assertions bucket A touched; this plan assumes A landed (42 → 45). If A has not landed, use 40 → 43 and drop the bucket-A membership assertions.
