...
> 1. **A/D collision on `v1-allowed.ts` (Task 4).** Gate Task 4 on bucket A's `v1-allowed.ts` commit; Layer 1 (provider+stub) may proceed in parallel. Add a blocking precondition step to Task 4: run `git log --oneline -- src/standalone/v1-allowed.ts` and read the **live** asserted count rather than trusting the literal "42". (A landed ⇒ 42→45; if not, compute current+3.)
...
- [ ] **Step 0 (blocking precondition — grilling decision 1): confirm bucket A landed and read the LIVE asserted count**

Layer 1 (provider + stub) may proceed in parallel with bucket A, but this task edits the **same** `v1-allowed.ts` set and the **same** size assertions bucket A touches — so gate it on bucket A's commit:

```bash
git log --oneline -- src/standalone/v1-allowed.ts
```

Then read the count currently asserted in `src/standalone/__tests__/v1-allowed.test.ts` — do **not** trust the literal "42" below. If it asserts **42**, bucket A landed → target **45**, keep the bucket-A membership assertions. If it asserts **40**, bucket A has not landed → target **43**, and drop the two bucket-A (`getDataExplorer`/`evaluateExpression`) membership assertions. Substitute the live number for every "42 → 45" in the steps below.
